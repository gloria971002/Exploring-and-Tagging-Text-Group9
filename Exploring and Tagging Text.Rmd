...
```{r message=FALSE, warning=FALSE}
# Distribution of sentences in chapter
co_occur_info %>% group_by(sentence) %>% 
  summarise(u_chapter=unique(chapter),
            times=n()) %>% 
  group_by(u_chapter) %>% 
...
ggplot() + 
  geom_bar(aes(x=reorder(u_chapter,sum_times),y=sum_times),stat = "identity") +
  geom_text(aes(x=reorder(u_chapter,sum_times),y=sum_times,label=sum_times),hjust=-0.5,col="royalblue") + 
  xlab("Times") + ylab("Chapter") + 
  ggtitle("Distribution of Sentences in Chapters: Elizabeth & Darcy",subtitle = "Count of Their Co-occurence, Top 10") +
  coord_flip()
```

The plots denotes that there is the largest count of 70 in sentences in chapter 70 when they appear together in the *Pride and Prejudice*.
...
```{r}
# Dataframe-money_df
qmoney <- tnum.query("*pride* has * = REGEXP(\"money\")")
...
# plot_tree_money2<-tnum.makePhraseGraphFromPathList(money_df$subject)
# tnum.plotGraph(plot_tree_money)
# tnum.plotGraph(plot_tree_money2)
```
```{r}
tnum.tagByQuery("*pride* has text == REGEXP(\"money\")", "reference:Group9money")
qmoney <- tnum.query("*pride* has text == REGEXP(\"money\")", max = 50)
#qmoney
money_df <- tnum.objectsToDf(qmoney)
View(money_df)
plot_money <- tnum.makePhraseGraphFromPathList(money_df$subject)
tnum.plotGraph(plot_money)
```

The tree plot shows that money seems not to be a word that occurs many times in the book. Only 7 chapters contain the word "money".
